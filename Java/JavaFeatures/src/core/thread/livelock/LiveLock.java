   ...
   public void run ()
   {
      while (true)
      {
         synchronized (bucket)
         {
            ...
            {
               Util.threadLog("[Bye] Bucket filled");
               break;
            }
            
            int num = (int)(Math.random()*100);
            bucket.add(new Integer(num));
            Util.threadLog(" Bucket " + bucket + " Add=" + num);
         }
   ...
   {      
      while (true)
      {
         synchronized (bucket)
         {
            if (bucket.isEmpty())
            {
               Util.threadLog("[Bye] Bucket emptied");
               break;
            }
            
            int num = bucket.remove(0);
            Util.threadLog(" Bucket " + bucket + " Removed=" + num);
         }
         Util.sleep(sleepTime);
      }
   }   
